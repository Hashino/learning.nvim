...
wait_win() { tui-use wait --text "to dismiss" "${1:-45000}" >/dev/null 2>&1; }
...
edit "Gcc    return sum(numbers)"
wait_win 45000
win_open && ok "auto-suggestion: meaningful edit opens a window" \
  || bad "auto-suggestion: meaningful edit opens a window"
dismiss

# --- dismissal suppression: same feature (comment) past threshold (2) ---
edit "ggO# alpha"
wait_win 45000
win_open && ok "suppression: comment suggestion #1 shows" \
  || bad "suppression: comment suggestion #1 shows"
dismiss
edit "ggO# beta"
wait_win 45000
win_open && ok "suppression: comment suggestion #2 shows" \
  || bad "suppression: comment suggestion #2 shows"
dismiss
edit "ggO# gamma"
settle 10000
win_open && bad "suppression: comment suppressed after threshold" \
  || ok "suppression: comment suppressed after threshold"

# --- explain a real visual selection -> relevant answer ---
# insert a distinctive construct; dismiss any auto-suggestion it triggers.
edit "Gosquares = [x * x for x in range(10)]"
wait_win 20000
dismiss
...
tui-use press enter >/dev/null 2>&1
wait_win 45000
# this asserts the real selection -> explain -> window PATH works (marks +
# visualmode the headless runner can't drive). content relevance is asserted
# against the raw response string in tests/run.lua, not by scraping the wrapped
# float, where words like "comprehension" wrap mid-word and defeat a flat search.
win_open && ok "explain: real visual selection opens an explanation window" \
...
edit "Goprint([n + 1 for n in numbers])"
settle 6000
...
tui-use press enter >/dev/null 2>&1
tui-use wait 500 >/dev/null 2>&1
edit "Goprint([n * 3 for n in numbers])"
wait_win 45000
win_open && ok "enable: suggestions resume after re-enabling" \
